...
struct RoomInfo: Codable {
    ...
    func setupUI() {
        ...
        NSLayoutConstraint.activate([
            playersTableView.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: 20),
            playersTableView.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 20),
            playersTableView.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -20),
            playersTableView.bottomAnchor.constraint(equalTo: startButton.topAnchor, constant: -20),
    ...
    func fetchRoomData() {
        ...
        URLSession.shared.dataTask(with: url) { [weak self] data, _, error in
            ...
            if let data = data {
                do {
                    let json = try JSON(data: data)
                    self.players = json["players"].arrayValue.map { $0.stringValue }
                    DispatchQueue.main.async {
                        self.playersTableView.reloadData()
                    }
                } catch {
                    print("Error parsing room data: \(error)")
                }
            }
        }.resume()
    }
    ...
    @objc func startGame() {
        ...
        let parameters: [String: Any] = ["room_code": roomCode]

        guard let url = URL(string: "https://api.areyousmarterthan.xyz/start_game") else {
            print("Invalid URL for starting game")
            return
        }
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, _, error in
            ...
            do {
                ...
                if json["success"].boolValue {
                    ...
                    print("Failed to start game: \(message)")
                    DispatchQueue.main.async {
                        self.statusLabel.text = message.isEmpty ? "Failed to start game." : message
                    }
                }
            } catch {
    ...
    func transitionToGame() {
        let triviaVC = TriviaViewController()
        triviaVC.roomCode = roomCode
        triviaVC.playerName = playerName
        triviaVC.modalPresentationStyle = .fullScreen
        triviaVC.modalTransitionStyle = .crossDissolve
